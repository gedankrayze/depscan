...
|---|---|
| Bun text lock v0/v1/v2/v3, JSONC, scoped names, aliases, workspaces, nested/version-scoped overrides, v2 integrity/Git safety, and malformed locators | `bun_lock.rs::{parses_current_bun_lock_locators_without_leaking_locator_prefixes,parses_bun_14_generated_v2_and_v3_override_resolutions,parses_version_one_bun_lockfiles,parses_version_zero_workspace_tuple_shape,rejects_malformed_bun_locator_arrays_with_context}` |
...
| DS-066 | platform-specific exact help snapshots, canonical cache-path assertion, complete CLI exit-code harness, and successful native Windows job `96373789695` (`68/68`) |
| DS-067 | canonical-root and component-normalized Cargo source snapshots, complete `cargo_manifest` integration harness, exact Clippy, and successful native Windows job `96373789695` (`13/13`) |
...
| DS-093 | release-workflow pin/count verifier and release-mode local gates; tag-driven Release run `32504708794` and five-platform public acceptance run `32505913430` passed with checkout v7.0.1 |
