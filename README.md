...
Node directness follows the same evidence boundary. npm v2/v3 root and validated workspace edges prove their installed targets direct, and only descendants reachable through installed-package edges are known transitive; disconnected records stay unknown. pnpm uses exact importer resolution locators, including aliases and peer-qualified snapshots. Yarn binds root/workspace manifest selectors to a unique lock descriptor group; unbound or ambiguous selectors stay unknown. When a format requires manifests, depscan reads only the root plus declared workspace manifests whose non-symlink final paths canonicalize inside the project—there is no depth cutoff and arbitrary nested `package.json` files are not treated as project manifests. Missing, malformed, or incomplete manifest evidence never turns unproven directness or development scope into a known negative, so conservative filters retain it.
...
Cargo-dist 0.32.0 generates the release plan and MSI definition. The checked-in workflow pins every external action by commit, runs formatting, all-target tests, strict Clippy, `cargo package --workspace --locked`, and a release-binary startup check before artifact jobs, then publishes the shell installer, Windows MSI, archives, source, and SHA-256 files. Host-phase GitHub artifact attestations bind every published file to the tag workflow. A release tag must resolve to the current protected `main` tip; release operations additionally require an active repository tag ruleset that prevents matching `v*` tags from being updated or deleted, and the host re-peels the remote tag before both draft creation and publication. The host verifies the complete checksum-bound inventory, creates a complete draft, and only then publishes it so repository-level release immutability can lock the tag and assets together. Workflow permissions default to read-only; only the host job receives `contents: write`, `id-token: write`, and `attestations: write`. `scripts/verify-release-workflow.sh` guards those pins, permissions, attestation scope, tag-target and remote-tag checks, immutable-ready publication order, and quality dependency.

After a release, verify a downloaded artifact before running it:

```bash
gh release download v2.0.2 --repo gedankrayze/depscan --pattern 'depscan-cli-aarch64-apple-darwin.tar.xz*'
...
The repository also includes a thin composite GitHub Action. It downloads the archive for the runner's supported OS/architecture from an exact release tag, validates the paired SHA-256 record before extraction, and passes only typed inputs to the CLI—there is no free-form shell argument input. The installer hands the runner the verified executable's exact absolute path through GitHub's environment file; the runner never searches the caller workspace or `PATH` for a downloaded `depscan` executable:
...
steps:
  ...
  - id: depscan
    uses: gedankrayze/depscan@v2.0.2
    with:
      version: v2.0.2
      path: .
      format: sarif
...
The `report` action output contains the configured report path. The action supports the five release runners listed above and fails closed on every other OS/architecture. The optional `binary` input is a deliberate local-testing override: its caller-provided path is resolved to an absolute path and treated as trusted, while an empty override requires the exact path exported by the verified installer. The action never enables `--allow-tools`; projects requiring Bun or dotnet execution must invoke the CLI explicitly after making that trust decision. Release-asset downloads are intentionally unauthenticated for the public zero-auth distribution model. The tag-dispatched acceptance workflow checks out the immutable release source and invokes that local action without the override on all five runners, so the real installer must download the public archive, verify its paired checksum, expose the exact installed binary, produce the four-package report, and preserve provider exit `30`. This repository requires full commit pins for non-local actions, so the self-acceptance workflow deliberately uses the immutable checked-out tag path rather than weakening that policy for a semver self-reference.
